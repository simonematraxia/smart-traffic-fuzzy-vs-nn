function [exectime, data] = controller_script(segment, data)
...
persistent log_msg_sent log_hours_passed log_fuzzy_timeS1_2 log_fuzzy_timeS3 log_red_timeS1_2 log_red_timeS3; % Log variables to measure system performance

persistent ds_X ds_Y    % Fuzzy dataset: inputs and outputs

persistent intergreen_countdown pending_green % All-red pause between alternating green phases
...
if isempty(log_msg_sent) 
    ...
    log_fuzzy_timeS1_2 = []; % Calculated times for S1 and S2
    log_fuzzy_timeS3 = []; % Calculated times for S3
            ...
            if(current_state == 1) % Normal state
                ...
                if strcmp(data.S1state, 'green') || strcmp(data.S1state, 'yellow') || strcmp(data.S2state, 'green') || strcmp(data.S2state, 'yellow')
                    ...
                    if data.S1duration == 6 && S1_2_total < MaxGreen_S12
                        ...
                        if ext == 5   % mid-range value, symptom of "no rules fired"
                        ...
                        ext = round(ext);   % Extension in seconds (0..10)
                        if ext > 0
                ...
                if strcmp(data.S3state, 'green') || strcmp(data.S3state, 'yellow')
                    ...
                    if data.S3duration == 6 && S3_total < MaxGreen_S3
                        ...
                        if ext == 5   % mid-range value, symptom of "no rules fired"
                            ...
                            %----------------@DEBUG@-------------------------------------------------------------------------------------------------------------------------
                            %fprintf('[%s][Controller] WARNING, queue_c=%.3f flow_c=%.3f queue_o=%.3f ext=%.3f\n', ttSimTimeStr(), queue_curr, flow_curr, queue_other, ext);
                            %------------------------------------------------------------------------------------------------------------------------------------------------
