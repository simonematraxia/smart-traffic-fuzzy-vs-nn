function [exectime, data] = controllore_code(segment, data)
...
persistent log_msg_sent log_hours_passed log_fuzzy_timeS1_2 log_fuzzy_timeS3 log_red_timeS1_2 log_red_timeS3; %variabili di Log per misurare le performance del sistema

persistent ds_X ds_Y    % dataset del fuzzy: input e output
...
if isempty(log_msg_sent) 
    ...
    log_fuzzy_timeS1_2 = []; % Tempi calcolati per S1 ed S2
    log_fuzzy_timeS3 = []; % Tempi calcolati per S3
            ...
            if(current_state == 1) %stato normale
                ...
                if strcmp(data.S1stato, 'verde') || strcmp(data.S1stato, 'giallo') || strcmp(data.S2stato, 'verde') || strcmp(data.S2stato, 'giallo')
                    ...
                    if data.S1durata == 6 && S1_2_total < MaxGreen_S12
                        ...
                        ext = evalfis(data.fis, [queue_curr, flow_curr, queue_other]);

                        if ext == 5   % valore medio del range, sintomo di "no rules fired"
                            
                            %----------------@DEBUG@----------------------------------------------------------------------------------
                            %fprintf('[%s][WARNING DEBUG] qc=%.3f fc=%.3f qo=%.3f ext=%.3f\n', ttSimTimeStr(), queue_curr, flow_curr, queue_other, ext);
                            %---------------------------------------------------------------------------------------------------------

                        end

                        % Salva un campione nel dataset per la NN (ramo S12)
                        ...
                        ext = round(ext);   % estensione in secondi (0..10)
                        if ext > 0
                    ...
                    elseif data.S1durata <= 0
                        ...
                        if ~isempty(S1_2_total)
                            log_fuzzy_timeS1_2 = [log_fuzzy_timeS1_2; S1_2_total];
                ...
                if strcmp(data.S3stato, 'verde') || strcmp(data.S3stato, 'giallo')
                    ...
                    if data.S3durata == 6 && S3_total < MaxGreen_S3
                        ...
                        if ext == 5   % valore medio del range, sintomo di "no rules fired"
                            ...
                            %----------------@DEBUG@-------------------------------------------------------------------------------------------------------------------------
                            %fprintf('[%s][Controllore] WARNING, queue_c=%.3f flow_c=%.3f queue_o=%.3f ext=%.3f\n', ttSimTimeStr(), queue_curr, flow_curr, queue_other, ext);
                            %------------------------------------------------------------------------------------------------------------------------------------------------

                        end

                        % Salva un campione nel dataset per la NN (ramo S3)
                        sample_in  = [queue_curr, flow_curr, queue_other]; 
                        sample_out = ext; % estensione effettiva usata
                    ...
                    elseif data.S3durata <= 0
                        % Logga il verde effettivo usato in questa fase
                        if ~isempty(S3_total)
                            log_fuzzy_timeS3 = [log_fuzzy_timeS3; S3_total];
